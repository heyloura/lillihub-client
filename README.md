...
# Running Locally
You will need to have [Deno](https://deno.com/) installed and available on your `$PATH`. Then run the command `deno run --allow-env --allow-net --allow-read --unstable-kv main.js` (or `deno task dev`). This will run the webserver on your `localhost`. There is no build step.

# Running tests
Smoke tests live in `tests/` and use fixture JSON to stub micro.blog API responses. Run them with `deno task test`. They should complete in a few seconds and confirm every layout renders without crashing.
