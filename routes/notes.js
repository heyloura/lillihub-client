...
export async function tryHandle(req, ctx) {
    ...
    // /notes/notebook/move matches /notes/:notebookId/:noteId
    ...
    if (NOTE_VERSIONS_API.exec(req.url) && user) {
        const searchParams = new URLSearchParams(req.url.split('?')[1]);
        const noteId = searchParams.get('id');
        if (!noteId) return new Response('{}', { status: 400, headers: { 'content-type': 'application/json' } });

        try {
            const versionsRes = await fetch(`https://micro.blog/notes/${noteId}/versions`, {
                method: 'GET',
                headers: { 'Authorization': 'Bearer ' + accessTokenValue }
            });
            ...
            return new Response('{"error":"failed"}', { status: 500, headers: { 'content-type': 'application/json' } });
        }
    }
