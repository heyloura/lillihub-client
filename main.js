                ...
                <script>
                    ...
                    window.createTaskList = function() {
                        ...
                        for (var i = 0; i < tasks.length; i++) {
                            ...
                            if(ids.length > 0) {
                                console.log(ids[0]);
                                markup = markup.replace(ids[0].toString(), '<span class="chip no-elevate">'+ids[0]+'</span>')
                            }
    ...
    function resize (html) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            if(images.length > 1)
            {
                ...
                image.setAttribute('data-ui', `#${image.getAttribute('src')}`);
                ...
                dialog.innerHTML = `<header class="fixed front">
                    <nav>
                        ...
                        <button data-ui="#${image.getAttribute('src')}" class="circle transparent"><i>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-lg" viewBox="0 0 16 16">
                                <path d="M2.146 2.854a.5.5 0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5 0 0 1 .708.708L8.707 8l5.147 5.146a.5.5 0 0 1-.708.708L8 8.707l-5.146 5.147a.5.5 0 0 1-.708-.708L7.293 8z"/>
                            </svg>
                        </i></button>
...
function postHTML(item, lastSeen, convoView = false) {
    item = flattenPost(item);
    return `
        <article class="no-elevate round ${lastSeen ? 'secondary-container' : ''}" data-id="${item.id}">
                    ...
                    <div>
                        <div class="row">
                            ...
                            <div class="max">
                                ...
                                <label class="">${item.username ? ` <a href="${item.author_url}">@${item.username}</a>` : ''}</label>
                            </div>
                        </div>
                            <div class="medium-space"></div>
                            <div class="post">
                            ...
                            <div class="clear">
                                <nav class="no-space ">
                                    <div class="max">
                                        <a class="" href="${item.url}">${item.date_relative}&nbsp;
                                            <i class="tiny">
                                    ...
                                    ${item.is_conversation && !convoView ? 
